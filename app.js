...
function renderSplash(){
  const lines=[
    ...
    "श्रीमत्पिङ्गलनागोक्तच्छन्दःशास्त्रमहोदधेः ।",
    "वृत्तानि मौक्तिकानीव कानिचिदिचिनोम्यहम् ॥ २ ॥"
  ];
...
/* ── Top-bar Search — searches sūtra + pada + tika ── */
function _searchSutras(q){
  ...
  return SUTRAS.filter(s=>{
    // Search in sūtra text, padaccheda, anuvṛtti, and tika — all normalised
    const fields = [s.s, s.pc, s.av, s.h].map(normDev);
    return fields.some(f => f.includes(qd) || f.includes(qi) || f.includes(ql));
  });
}
...
async function submitFeedback(){
  ...
  if(!msg){ alert('कृपया सन्देशं लिखतु। Please write your message.'); return; }

  const btn = document.querySelector('#view-feedback button[onclick="submitFeedback()"]');
  const originalText = btn ? btn.innerHTML : '';
  if(btn){ btn.disabled = true; btn.innerHTML = 'भेज्यते… Sending…'; }
  ...
  try {
    const res = await fetch('https://formspree.io/f/YOUR_FORM_ID', {
      ...
      body: JSON.stringify({
        name:    name || '(अनाम · Anonymous)',
        message: msg,
        _subject: 'Feedback — पिङ्गलछन्दःशास्त्रम्'
      })
    });
    if(res.ok){
      ...
      alert('सन्देशः न प्राप्तः। Message failed. Please try again.');
    }
  } catch(e){
    alert('नेटवर्क-दोषः। Network error. Please check your connection.');
  } finally {
